...
void CACHE::handle_writeback()
{
    while (writes_available_this_cycle > 0)
    {
        ...
        if (way < NUM_WAY) // HIT
        {
            ...
            if(cache_type == IS_LLC && handle_pkt.type == TRANSLATION)
                translation_access[set*NUM_WAY] += 1;
        ...
        else // MISS
        {
            ...
            if (cache_type == IS_L1D) {
                ...
                auto set_end   = std::next(set_begin, NUM_WAY);
                auto first_inv = std::find_if_not(set_begin, set_end, is_valid<BLOCK>());
                // Find factors that do not meet the conditions within the range. If there is no factor that meets the condition, return the last value.
...
void CACHE::handle_read()
{
    while (reads_available_this_cycle > 0) {
        ...
        BLOCK &fill_block = block[set*NUM_WAY + way]; // add code
...
bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET &handle_pkt)
{
    ...
    if(cache_type == IS_LLC && set_begin->pin_cnt > pin_threshold[set])
    {
        uint32_t pin_way;
